...
export function CodeBlock({
  ...
  return (
    <div className={cn("overflow-x-auto bg-editor font-mono", className)}>
      <pre className={cn("min-w-max", compact ? "py-2 text-xs" : "py-4 text-[13px]")}>
        <code>
          {lines.map((line, i) => (
            <div
              key={i}
              className={cn(
                "flex leading-6",
                highlight.includes(i + 1) && "bg-primary/10",
              )}
            >
              <span className="w-11 shrink-0 select-none pr-4 text-right text-muted-foreground/50">
                {i + 1}
              </span>
